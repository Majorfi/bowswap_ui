...
function	Navbar({hasSecret, shouldInitialPopup}) {
	...
	const	[currentAddress, set_currentAddress] = useState('');

	React.useEffect(() => set_currentAddress(address), [address]);
	React.useEffect(() => set_isActive(active), [active]);
	...
	function	renderWalletButton() {
		if ((currentAddress && isActive) || (currentAddress && !provider)) 
			return (
				<p
