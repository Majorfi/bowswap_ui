...
function	FAQElement({label, children}) {
	...
	function	onExpand() {
		if (isExpanded) {
			...
			setTimeout(() => set_isExpanded(false), 400);
		} else {
			set_isExpanded(true);
	...
	return (
		<AnimateHeight duration={400}>
			<div  className={'bg-white rounded-lg w-full cursor-pointer'}>
				...
				<div className={`bg-lightDark w-full transition-max-height overflow-hidden ${isExpandedAnimation ? 'max-h-96' : 'max-h-0'}`}>
	...
	return (
		<div className={'space-y-4 mt-6 w-full'}>
			...
			<FAQElement label={'What is Bowswap?'}>
				<div className={'px-6 pb-6 font-normal text-ygray-700 text-xs leading-4'}>
			...
			<FAQElement label={'What are the benefits of using Bowswap?'}>
				<div className={'px-6 pb-2 font-normal text-ygray-700 text-xs leading-4'}>
