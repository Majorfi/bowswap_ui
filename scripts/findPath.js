...
async function withBacktracking({from, fromList, visitedFrom, to, toList, path, i, max}) {
	if (path.length > 0) {
		...
		const	firstIsValid = (allPools[first[1]]?.address === from) || (allPools[first[1]]?.coins || []).includes(from) || allPools[first[1]]?.lpToken === from;
		...
		if (whereFromIs.length > 0) {
			for (let index = 0; index < whereFromIs.length; index++) {
				...
				if (element.lpToken) {
					...
					if (path.find(([, addr]) => addr === element.address)) {
						continue;
					}
...
async function findPath({from, to}) {
	...
	const	fromList = [fromToken, ...allPools[fromToken]?.coins || '', ...fromAsLPToken?.coins || ''];
	const	toList = [toToken, ...allPools[toToken]?.coins || '', ...toAsLPToken?.coins || ''];
	
	let	returnValue = undefined;
	for (let max = 1; max < 5; max++) {
		const	result = await withBacktracking({from: fromToken, fromList, visitedFrom: [], to: toToken, toList, path: [], i: 0, max: max});
		if (result !== false) {
			// console.log(result);
			returnValue = result;
			break;
...
// async function findPathAlt({from, to}) {
// 	const	fromAsLPToken = allPoolsAsArray.find(e => e.lpToken === from);
// 	const	toAsLPToken = allPoolsAsArray.find(e => e.lpToken === to);
// 	const	fromList = [from, ...allPools[from]?.coins || '', ...fromAsLPToken?.coins || ''];
// 	const	toList = [to, ...allPools[to]?.coins || '', ...toAsLPToken?.coins || ''];
	
// 	let	returnValue = undefined;
// 	// for (let max = 1; max < 5; max++) {
// 	const	result = await withBacktracking({from: from, fromList, visitedFrom: [], to: to, toList, path: [], i: 0, max: 3});
// 	if (result !== false) {
// 		console.log(result);
// 		returnValue = result;
// 		// break;
...
async function	findAllPath() {
	...
	for (let vaultIndex = 0; vaultIndex < VAULT_ADDRESSES.length; vaultIndex++) {
		...
		for (let secondVaultIndex = 0; secondVaultIndex < VAULT_ADDRESSES.length; secondVaultIndex++) {
			...
			if (result) {
				console.log({from: element, to: secondElement, path: result});
			} else {
				console.log({from: element, to: secondElement, path: []});
			}
		}
	}
}
...
// findPathAlt({from: '0x43b4FdFD4Ff969587185cDB6f0BD875c5Fc83f8c', to: '0x075b1bb99792c9E1041bA13afEf80C91a1e70fB3'});
